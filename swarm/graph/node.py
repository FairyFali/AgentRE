...
class Node(ABC):
    ...
    def process_input(self, inputs):
        ...
        all_inputs = []
        if inputs is None:  # 这里的逻辑有问题，为什么不直接合并拿到前置节点的输出
    ...
    async def execute(self, **kwargs):
        ...
        if not self.inputs and self.predecessors:
            if self.combine_inputs_as_one:
                ...
                tasks.append(asyncio.create_task(self._execute(combined_inputs, **kwargs)))  # tasks是一系列返回的结果
                # print('### combined inputs', combined_inputs, '\n')
            else:
                predecessor_outputs = []
                for predecessor in self.predecessors:
                    predecessor_outputs = predecessor.outputs
                    if isinstance(predecessor_outputs, list) and predecessor_outputs:
                        for predecessor_output in predecessor_outputs:
                            ...
                            predecessor_outputs.append(predecessor_output)
                # print('### predecessor outputs', predecessor_outputs, '\n')
        elif self.inputs:
            # fali added
            if self.predecessors:
                combined_inputs = []
                for predecessor in self.predecessors:
                    ...
                    if predecessor_outputs is not None and isinstance(predecessor_outputs, list):
                        combined_inputs.extend(predecessor_outputs)
                tasks = [asyncio.create_task(self._execute(input, combined_inputs, **kwargs)) for input in self.inputs]
            ###
            else:
        ...
        if tasks:
            try:
                ...
                # 打完整堆栈到日志
                logger.exception(f"Node {type(self).__name__} failed to execute (id={self.id})")
                # 让异常继续冒泡，别吞
                raise
    ...
    @abstractmethod
    async def _execute(self, input, predecessor_outputs, **kwargs):
