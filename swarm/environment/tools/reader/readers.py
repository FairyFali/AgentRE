...
class JSONLReader(Reader):
    def parse_file(file_path: Path) -> List[Any]:
        """逐行读取 JSONL 文件；每行一个合法 JSON 对象。"""
        logger.info(f"[JSONLReader] Reading JSON Lines from {file_path}")
        records: List[Any] = []
        with open(file_path, "r", encoding="utf-8") as f:
            while True:
                ...
                if not raw_line:          # EOF
                    break
                line = raw_line.strip()   # 去掉行尾 \n / \r\n
                if not line:              # 跳过空行
                    continue
                records.append(json.loads(line))

        logger.info(f"[JSONLReader] Loaded {len(records)} records\n")
        return records
