...
load_dotenv()
DEEPSEEK_API_KEYS=os.getenv(f"DEEPSEEK_API_KEY")
...
def gpt_chat(
    ...
    if model == "lmstudio":
        ...
        api_kwargs = dict(base_url="https://ark.cn-beijing.volces.com/api/v3",api_key=DEEPSEEK_API_KEYS)
        client = OpenAI(**api_kwargs)
        model = 'ep-20250322232258-pfwh6'
    elif model == "deepseek-r1":
        api_kwargs = dict(base_url="https://ark.cn-beijing.volces.com/api/v3",api_key=DEEPSEEK_API_KEYS)
        client = OpenAI(**api_kwargs)
        model = 'ep-20250322232225-2kx69'
    else:
