...
async def main():
    ...
    # model_name: Optional[list] = args.model_names
    ...
    dataset_train = MATHDataset('train')
    dataset_val = MATHDataset('test')
    ...
    if mode == 'DepthParallelSwarm':
        ...
        graph = await evaluator.optimize_llm_swarm(num_iters=num_iters, model_selection_list=model_selection_list,optimizer=optimizer, budget=budget)
        ...
        graph = await evaluator.optimize_random_swarm(num_iters=num_iters, model_selection_list=model_selection, budget=budget)
        ...
        graph = await evaluator.optimize_textgrad_swarm(num_iters=num_iters, model_selection=model_selection,optimizer=optimizer, budget=budget)
